...
print("CLIENT IN MAIN:", id(client))
@client.on(events.NewMessage(outgoing=True))
async def debug(event):
    print("HANDLER CLIENT:", id(client))
...
@client.on(events.NewMessage(pattern=r'/GUnSilent(\s+\d+)?'))
...
@client.on(events.NewMessage(pattern=r'/flood (\d+) - ([\w,]+)'))
...
async def main():
    ...
    # Start updater loops
    asyncio.create_task(update_last_name(client))
    asyncio.create_task(update_about(client))
    asyncio.create_task(update_first_name(client))
    asyncio.create_task(update_profile_photo(client))
